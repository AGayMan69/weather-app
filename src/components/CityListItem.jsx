...
const weatherIconMap = {
  ...
  clear: SunnyIcon,
  cloudy: CloudyIcon,
  "partly cloudy": PartlyCloudyDayIcon,
  rain: RainyIcon,
  showers: RainyIcon,
  snow: SnowyIcon,
...
const WeatherIconDisplay = ({ condition, size = 38 }) => {
  const iconKey =
    typeof condition === "string" ? condition.toLowerCase() : "default";
  const IconComponent = weatherIconMap[iconKey] || weatherIconMap["default"];
  return IconComponent ? (
    <IconComponent width={size} height={size} className="weather-svg-icon" />
  ) : null;
};
...
function CityListItem({ city, onCitySelect }) {
  ...
  return (
    <li
      ...
      tabIndex={0}
      role="button"
      aria-label={`Select ${city.name}, ${locationString}`}
    >
      ...
      <div className="weather-info">
        <WeatherIconDisplay
          condition={city.weatherCondition || "default"}
          size={38}
        />

...
const getIconConditionKey = (weather) => {
    ...
    switch (main) {
        ...
        case 'drizzle': case 'rain': return 'rainy';
        case 'snow': return 'snowy';
        case 'clear': return iconCode.includes('n') ? 'clear-night' : 'sunny';
        case 'clouds':
            ...
            return 'cloudy';
        case 'mist': case 'smoke': case 'haze': case 'dust': case 'fog': case 'sand': case 'ash': return 'cloudy';
        case 'squall': case 'tornado': return 'windy';
...
const getAqiDescription = (aqiValue) => {
    ...
    switch (aqiValue) {
        ...
        case 4: return 'Poor'; case 5: return 'Very Poor'; default: return '';
    }
};
...
const getUvDescription = (uvi) => {
    ...
    if (uviRounded <= 2) return 'Low'; if (uviRounded <= 5) return 'Moderate';
    if (uviRounded <= 7) return 'High'; if (uviRounded <= 10) return 'Very High';
    return 'Extreme';
};
...
function transformOpenWeatherData(apiData, locationName = 'Selected Location') {
    ...
    if (!apiData || !apiData.current || !apiData.forecast || !apiData.forecast.list) {
       ...
       console.log("--- END Transformation (Error) ---"); return null;
    }

    const { current, forecast, aqiData, uvData, imageUrl } = apiData; // Include imageUrl
    const timezoneOffsetSeconds = current.timezone;

    const getLocalDate = (unixTimestamp) => new Date((unixTimestamp + timezoneOffsetSeconds) * 1000);
    ...
    const formatDay = (d, i) => i === 0 ? 'Today' : d.toLocaleDateString('en-US', { weekday: 'short' });
    const formatHour = (d) => d.toLocaleTimeString('en-US', { hour: 'numeric', hour12: true }).replace(' ', '');
    ...
    forecast.list.forEach(item => {
        ...
        if (itemTimestamp > nowTimestamp && hourly.length < 6) {
            hourly.push({ time: formatHour(itemDate), temp: Math.round(item.main.temp), icon: getIconConditionKey(item.weather) });
        }
        const dateKey = itemDate.toISOString().split('T')[0];
        if (!dailyAggData[dateKey]) { dailyAggData[dateKey] = { temps: [], pop: 0, weather: [], dt: item.dt }; }
        dailyAggData[dateKey].temps.push(item.main.temp);
        dailyAggData[dateKey].weather.push(item.weather[0]);
        if (item.pop > dailyAggData[dateKey].pop) { dailyAggData[dateKey].pop = item.pop; }
    });
    const daily = Object.keys(dailyAggData).sort().slice(0, 6).map((dateKey, index) => {
        const dayData = dailyAggData[dateKey]; const dayDate = getLocalDate(dayData.dt);
        const dayMinTemp = Math.round(Math.min(...dayData.temps)); const dayMaxTemp = Math.round(Math.max(...dayData.temps));
        let dominantIcon = 'default'; const conditions = dayData.weather.map(w => getIconConditionKey([w]));
        ...
        return { day: formatDay(dayDate, index), temp: dayMaxTemp, icon: dominantIcon, isActive: index === 0, tempMin: dayMinTemp, tempMax: dayMaxTemp };
    });
    ...
    const result = {
        location: current.name || locationName, date: formatDate(currentDate), temp: currentTemp, condition: currentCondition, icon: currentIconKey,
        wind: windSpeed, humidity: humidityValue, precip: Math.round((dailyAggData[Object.keys(dailyAggData)[0]]?.pop || 0) * 100),
        uvIndex: uvIndexDisplay, uvDesc: uvDescDisplay, tempMax: finalTempMax, tempMin: finalTempMin,
        sunrise: formatTime(sunriseDate), sunset: formatTime(sunsetDate), feelsLike: feelsLikeTemp,
        aqi: aqiDisplay, aqiStatus: aqiStatusDisplay, hourly: hourly, daily: daily,
...
function App() {
    ...
    useEffect(() => {
        if (!navigator.geolocation) {
            ...
            setSelectedLocation(DEFAULT_LOCATION); return;
        }
        navigator.geolocation.getCurrentPosition(
            (pos) => { console.log("Geo success:", pos.coords); setSelectedLocation({ lat: pos.coords.latitude, lon: pos.coords.longitude, name: "Your Location" }); },
            (err) => { console.warn(`Geo Error (${err.code}): ${err.message}. Using default.`); setSelectedLocation(DEFAULT_LOCATION); },
    ...
    useEffect(() => {
        if (!selectedLocation?.lat || !selectedLocation?.lon || !API_BASE_URL) {
            if (isLoading && selectedLocation === null) { console.log("Waiting for location..."); return; }
            console.log("Skipping fetch: Invalid location/URL."); setIsLoading(false);
            setError(prev => prev || "Could not determine location."); setWeatherData(null); return;
        }
        const fetchWeather = async () => {
            if (!isLoading) setIsLoading(true); setError(null);
            console.log(`Fetching weather: ${selectedLocation.name} (Lat=${selectedLocation.lat}, Lon=${selectedLocation.lon})`);
            try {
                ...
                if (data) setWeatherData(data); else throw new Error("Transform failed.");
            } catch (err) {
                console.error("Fetch/process error:", err);
                let msg = "Could not load weather.";
                if (err.response?.data?.message || typeof err.response?.data === 'string') msg = `Error: ${err.response.data.message || err.response.data}`;
                else if (err.message) msg = err.message;
                setError(msg); setWeatherData(null);
    ...
    const handleCitySelection = (city) => {
        ...
        else { console.error("Invalid city selected:", city); setError("Invalid city data."); }
    };
